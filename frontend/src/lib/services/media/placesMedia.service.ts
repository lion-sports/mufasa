...
export default class PlacesMediaService extends FetchBasedService {
    ...
    try {
      ...
      blob = await this.client.getBlob({
        url: '/places/media/' + params.mediaId + '/download'
      })
  ...
  public async getThumbnailBlob(params: { mediaId: number }): Promise<Blob | undefined> {
    try {
      ...
      blob = await this.client.getBlob({
        url: '/places/media/' + params.mediaId + '/downloadThumbnail'
      })
	...
	public async get(params: { id: number }): Promise<Media> {
		let media: Media = await this.client.get({
			url: '/places/media/' + params.id + '/show'
		})
