...
export default class MediaService extends FetchBasedService {
    ...
    try {
      ...
      blob = await this.client.getBlob({
        url: '/media/' + params.mediaId + '/download'
      })
  ...
  public async getThumbnailBlob(params: { mediaId: number }): Promise<Blob | undefined> {
    try {
      ...
      blob = await this.client.getBlob({
        url: '/media/' + params.mediaId + '/downloadThumbnail'
      })
