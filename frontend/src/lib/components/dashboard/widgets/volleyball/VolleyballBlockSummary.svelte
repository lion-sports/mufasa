<script lang="ts">
	...
	import type { TotalBlockByPlayerResult, TotalBlockResult, TotalServeByPlayerResult, TotalServeResult, TotalSpikeForPlayerAndPositionResult, TotalSpikeForPlayerResult, TotalSpikeForPositionResult } from "@/lib/services/scouts/scoutAnalysis.service"
  ...
  $: if(!!widget.data?.totalBlock) {
    ...
    for(let i = 0; i < widget.data.totalBlock.length; i += 1) {
      ...
      data.datasets[datasetIndex].data = [ totalBlockRow.points, totalBlockRow.handsOut, totalBlockRow.putBack, totalBlockRow.touch]
    }
  }

  let settingsOpened: boolean = false
  let selectedTeamFilter: TeamFilter | undefined = undefined
  $: selectedTeamFilter = widget.widgetSetting?.settings?.widget == 'VolleyballBlockSummary' ? widget.widgetSetting?.settings.team : undefined
...
<div class="@container w-full h-full rounded-md max-h-full">
  ...
  <div class="h-[calc(100%-24px)] w-full grid grid-cols-1 @md:grid-cols-12 gap-3">
    ...
    <div class="@md:col-span-6 p-2 @md:h-[350px]">
      ...
      {:else}
        {#if !!widget.data?.totalBlockByPlayer && widget.data.totalBlockByPlayer.length > 0}
          <div class="flex flex-col gap-2 h-full overflow-auto">
            {#each widget.data.totalBlockByPlayer as playerStats}
              {@const previousStat = widget.data.previousTotalBlockByPlayer.find((e) => e.player.id == playerStats.player.id)}
              ...
              {@const previousStatHandsOutPercentageDifference = Number((playerStats.handsOutPercentage - (previousStat?.handsOutPercentage || 0)).toFixed(0))}
              {@const previousStatPutBackPercentageDifference = Number((playerStats.putBackPercentage - (previousStat?.putBackPercentage || 0)).toFixed(0))}
              {@const previousStatTouchPercentageDifference = Number((playerStats.touchPercentage - (previousStat?.touchPercentage || 0)).toFixed(0))}
              {@const previousStatPointsDifference = Number((playerStats.points - (previousStat?.points || 0)).toFixed(0))}
              {@const previousStatHandsOutDifference = Number((playerStats.handsOut - (previousStat?.handsOut || 0)).toFixed(0))}
              {@const previousStatPutBackDifference = Number((playerStats.putBack - (previousStat?.putBack || 0)).toFixed(0))}
                  ...
                  <div class="grid grid-cols-2">
                    ...
                    <div class="flex flex-col gap-1">
                      ...
                      <div class="mb-1 flex gap-2">
                        ...
                        {#if previousStat !== undefined}
                          <ValueChangeIndicator
                            difference={previousStatPutBackPercentageDifference}
                          ></ValueChangeIndicator>
                        {/if}
                      </div>
                    </div>
