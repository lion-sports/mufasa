<script lang="ts">
	...
	run(() => {
		selectedTeamFilter =
			widget.widgetSetting?.settings?.widget == 'VolleyballReceiveSummary'
...
<div class="@container w-full h-full rounded-md max-h-full">
	<div class="flex justify-between h-[24px]">
		<div class="text-lg font-bold">Receive summary</div>
		<div>
	...
	<div class="h-[calc(100%-24px)] w-full grid grid-cols-1 @md:grid-cols-12 gap-3">
		...
		<div class="@md:col-span-6 p-2 @md:h-[350px]">
			...
			{:else if !!widget.data?.totalReceiveByPlayer && widget.data.totalReceiveByPlayer.length > 0}
				<div class="flex flex-col gap-2 h-full overflow-auto">
					{#each widget.data.totalReceiveByPlayer as playerStats}
						{@const previousStat = widget.data.previousTotalReceiveByPlayer.find(
							(e) => e.player.id == playerStats.player.id
						)}
						{@const previousStatPerfectPercentageDifference = Number(
							(playerStats.perfectPercentage - (previousStat?.perfectPercentage || 0)).toFixed(0)
						)}
						{@const previousStatPlusPercentageDifference = Number(
							(playerStats.plusPercentage - (previousStat?.plusPercentage || 0)).toFixed(0)
						)}
						{@const previousStatMinusPercentageDifference = Number(
							(playerStats.minusPercentage - (previousStat?.minusPercentage || 0)).toFixed(0)
						)}
						{@const previousStatSlashPercentageDifference = Number(
							(playerStats.slashPercentage - (previousStat?.slashPercentage || 0)).toFixed(0)
						)}
						{@const previousStatErrorPercentageDifference = Number(
							(playerStats.errorPercentage - (previousStat?.errorPercentage || 0)).toFixed(0)
						)}
						{@const previousStatPerfectDifference = Number(
							(playerStats.perfect - (previousStat?.perfect || 0)).toFixed(0)
						)}
						{@const previousStatPlusDifference = Number(
							(playerStats.plus - (previousStat?.plus || 0)).toFixed(0)
						)}
						{@const previousStatMinusDifference = Number(
							(playerStats.minus - (previousStat?.minus || 0)).toFixed(0)
						)}
						{@const previousStatSlashDifference = Number(
							(playerStats.slash - (previousStat?.slash || 0)).toFixed(0)
						)}
						{@const previousStatErrorDifference = Number(
							(playerStats.error - (previousStat?.error || 0)).toFixed(0)
						)}
									...
									<div class="grid grid-cols-5 gap-1">
										<div class="mb-1 flex gap-2">
											...
											{#if previousStat !== undefined}
												<ValueChangeIndicator difference={previousStatPerfectDifference}
										...
										<div class="mb-1 flex gap-2">
											+: {playerStats.plus}
											{#if previousStat !== undefined}
												<ValueChangeIndicator difference={previousStatPlusDifference}
												></ValueChangeIndicator>
											{/if}
										</div>
										<div class="mb-1 flex gap-2">
