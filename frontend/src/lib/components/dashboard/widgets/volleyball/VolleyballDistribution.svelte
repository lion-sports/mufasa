<script lang="ts">
	...
	function getPositionStats(params: {
		...
		return widget.data?.totalSpikeForPosition.find(
			(e) =>
				Number(e.position) === Number(params.position) &&
				(!params.teamFilter ||
					(params.teamFilter == 'friend' && !e.opponent) ||
					(params.teamFilter == 'opponent' && e.opponent))
		)
	}

	let selectedPosition: VolleyballScoutEventPosition | undefined = $state(undefined)
	function handlePositionClick(params: { position: number }) {
		let position = params.position as VolleyballScoutEventPosition
		selectedPosition = position
	}

	let totalSpike = $derived(
		widget.data?.totalSpikeForPlayer.reduce((p, c, i, a) => {
			return p + c.total
		}, 0) || 0
	)
	...
	let selectedPositionStats = $derived(
		!!selectedPosition
			? widget.data?.totalSpikeForPlayerAndPosition.filter((el) => el.position === selectedPosition)
			: undefined
	)
	let selectedPositionPoints = $derived(
		!!selectedPositionStats
			? selectedPositionStats.reduce((p, c, i, a) => {
					return p + c.points
				}, 0)
			: undefined
	)
	let selectedPositionErrors = $derived(
		!!selectedPositionStats
			? selectedPositionStats.reduce((p, c, i, a) => {
					return p + c.errors
				}, 0)
	...
	run(() => {
		selectedTeamFilter =
			widget.widgetSetting?.settings?.widget == 'VolleyballDistribution'
...
<div class="@container w-full h-full rounded-md max-h-full">
	<div class="flex justify-between h-[24px]">
		<div class="text-lg font-bold">Distribution</div>
		<div>
	...
	<div class="h-[calc(100%-24px)] w-full grid grid-cols-1 @md:grid-cols-12 gap-3">
		<div class="@md:col-span-6 @2xl:col-span-4 p-2">
			...
			{:else}
				<div
					class="border border-[rgb(var(--global-color-contrast-300),.3)] h-full min-h-[300px] max-h-[350px]"
				>
					<div
						class="h-1/2 grid grid-cols-3 border-b border-[rgb(var(--global-color-contrast-300),.3)]"
					>
							...
							>
								{#if !widget.widgetSetting?.settings?.team || widget.widgetSetting?.settings?.team == 'both'}
									...
									<div class="font-light text-xs">
										# {friendsStat?.total || 0} vs # {opponentsStat?.total || 0}
									</div>
								{:else}
									<div
										class="absolute"
										...
										style:left="0px"
										style:right="0px"
										style:background-color={widget.widgetSetting?.settings?.team == 'friend'
											? 'rgb(59, 130, 246, .3)'
											: 'rgb(239, 68, 68, .3)'}
										style:z-index="-5"
									></div>
									<div class:font-bold={!!stats}>{stats?.percentage.toFixed(2) || 0} %</div>
		...
		<div class="@md:col-span-6 @2xl:col-span-8 p-2 @md:h-[320px]">
								...
								<tbody>
									{#each selectedPositionStats as positionStat}
										<tr class="hover:bg-[rgb(var(--global-color-background-200))] transition-all">
											<td
												class="py-2 pr-2 flex items-center gap-2 sticky left-0 bg-[rgb(var(--global-color-background-100))] z-10"
											>
												<div>
													<PlayerMarker
			...
			{:else if !!widget.data?.totalSpikeForPlayer && widget.data.totalSpikeForPlayer.length > 0}
				<div class="mb-4">
					<HorizontalStackedProgress
						progresses={[
							{
				...
				<div class="flex flex-col gap-2 h-[calc(100%-52px)] overflow-auto pt-2">
					{#each widget.data?.totalSpikeForPlayer as playerStats}
						{@const previousStat = widget.data.previousTotalSpikeForPlayer.find(
							(e) => e.player.id == playerStats.player.id
						)}
						{@const previousStatPercentageDifference = Number(
							(playerStats.percentage - (previousStat?.percentage || 0)).toFixed(0)
						)}
							...
							<div class="flex flex-col flex-grow">
								...
								<div class="mb-1 flex gap-2">
									...
									{#if previousStat !== undefined}
										<ValueChangeIndicator difference={previousStatPercentageDifference} postfix="%"
