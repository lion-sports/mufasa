  ...
  <svelte:fragment slot="item" let:item>
    <div class="item-container">
      <div class="times-column" style:color="rgb(var(--global-color-contrast-300))">
        {#if item.from}
          {DateTime.fromJSDate(item.from).setLocale('it').toLocaleString({day: 'numeric', month: 'short', hour:'2-digit', minute:'2-digit'})}
        {/if}
        {#if item.to}
          - {DateTime.fromJSDate(item.to).setLocale('it').toLocaleString({day: 'numeric', month: 'short', hour:'2-digit', minute:'2-digit'})}
        {/if}
      </div>
      <div class="info-column">
        <div class="title">{itemTitle}<span style:font-weight="700" style:margin-left="5px">{item.name}</span></div>
