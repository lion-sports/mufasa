	...
	run(() => {
		if (!!studio.selectedPlayer && !!studio.scout.stash?.playersServePositions) {
			...
			for (const [pos, v] of Object.entries(positionToLoop)) {
				if (!!v?.player && v.player.id == studio.selectedPlayer.id)
					selectedPlayerFieldPosition = pos as any as VolleyballScoutEventPosition
			}
		}
	})

	let cardBackgroundClass = $derived(
		studio.selectedPlayer?.isOpponent ? 'bg-red-500/15' : 'bg-blue-500/15'
	)
			...
			<div class="flex flex-col">
				...
				<div class="w-full {cardBackgroundClass} p-2 rounded-sm transition-all">
					<div class="flex flex-col gap-2">
						...
						<div class="flex justify-between gap-2">
							...
							<div class="font-bold">
								{PlayersService.translateRole(studio.selectedPlayer?.role)}
							</div>
						</div>
						<div class="flex justify-between gap-2">
				...
				<div class="w-full {cardBackgroundClass} p-2 rounded-sm transition-all">
					{#if !!studio.selectedPlayer}
						<StudioPlayerSubstitutionSuggestion player={studio.selectedPlayer} scout={studio.scout}
