	...
	run(() => {
		if (!!scout.stash?.currentSetSubstitutionsMade) {
			...
			for (let i = 0; i < scout.stash.currentSetSubstitutionsMade.enemy.length; i += 1) {
				...
				let closedSub = scout.stash.currentSetSubstitutionsMade.enemy.find(
					(s) => s.playerOut.id == sub.playerIn.id
				)

				substituted = [...substituted, sub.playerOut]

				if (!!closedSub) closedSubstitutions = [...closedSubstitutions, sub.playerIn]
			}
			...
			for (let i = 0; i < scout.stash.currentSetSubstitutionsMade.friends.length; i += 1) {
				...
				let closedSub = scout.stash.currentSetSubstitutionsMade.friends.find(
					(s) => s.playerOut.id == sub.playerIn.id
				)
...
>
	{#if !!changePlayer}
		<div class="flex flex-col p-2 w-[min(90vw,500px)]">
			{#each insidePlayers.filter((ip) => ip.isOpponent == changePlayer?.isOpponent && ((changePlayer.isOutForLibero && ip.role === 'libero') || (!changePlayer?.isOutForLibero && ip.role !== 'libero'))) as player}
				<button
				...
				>
					<div>
						<PlayerMarker friend={!changePlayer.isOpponent} opponent={changePlayer.isOpponent}
