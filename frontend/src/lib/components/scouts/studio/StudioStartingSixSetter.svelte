<script lang="ts">
	...
	let notSelectedFriendsPlayer = $derived(
		friendsPlayer.filter((fp) => {
			return (
				!playersPosition ||
				Object.values(playersPosition.friends)
					.filter((v) => !!v && !!v.player)
					.every((p) => Number(p.player.id) !== Number(fp.id))
			)
		})
	)

	let notSelectedOpponentsPlayer = $derived(
		opponentsPlayer.filter((fp) => {
			return (
				!playersPosition ||
				Object.values(playersPosition.enemy)
					.filter((v) => !!v && !!v.player)
					.every((p) => Number(p.player.id) !== Number(fp.id))
			)
		})
		...
		if (!!playersPosition) {
			...
			for (let i = 0; i < ORDERED_POSITIONS.length; i += 1) {
				...
				let suggestions = suggestPlayer({
					...
					avoidPlayers: Object.values(playersPosition.friends)
						.filter((v) => !!v && !!v.player)
						.map((v) => v.player)
				})
				...
				suggestions = suggestPlayer({
					...
					avoidPlayers: Object.values(playersPosition.enemy)
						.filter((v) => !!v && !!v.player)
						.map((v) => v.player)
				})
	...
	function dispatchAllPlayers(params: { friends?: boolean; enemy?: boolean }) {
		if (!!playersPosition) {
			if (params.enemy) {
	...
	function getPlayerValueInPosition(
		position: VolleyballScoutEventPosition,
		playersPosition: VolleyballPlayersPosition | undefined
	): ScoutEventPlayer[] {
...
<div class="min-w-[min(95vw,400px)] w-[700px] max-w-[95vw]">
	...
	<div class="mt-4 max-h-[80vh] overflow-auto">
		<div class="flex gap-4">
			...
			<button class="underline" onclick={handleRotateForward}> Ruota in avanti </button>
			<button class="underline" onclick={handleRotateBackward}> Ruota indietro </button>
				...
				<div class="p-2 flex gap-4 items-center">
					...
					<div class="flex-grow">
						<ScoutPlayerAutocomplete
							...
							players={selectedTab == 'opponents'
								? notSelectedOpponentsPlayer
								: notSelectedFriendsPlayer}
							onchange={(e) => {
					...
					{#if showSuggestion}
						<div class="flex flex-col gap-1">
							{#each selectedTab == 'opponents' ? playerSuggestion.enemy[position] || [] : playerSuggestion.friends[position] || [] as suggestion}
								<button
									class="flex gap-2 items-center"
									onclick={() => handleAcceptSuggestion(position, suggestion)}
								>
									<PlayerMarker opponent={suggestion.isOpponent} friend={!suggestion.isOpponent}
										>{suggestion.shirtNumber}</PlayerMarker
									>
									<div class="flex flex-col">
										<div class="text-left text-lg font-semibold">
											{TeammatesService.getTeammateName({
												teammate: suggestion.teammate,
												player: suggestion
											})}
										</div>
