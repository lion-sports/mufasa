<script lang="ts">
	...
	run(() => {
		if (!!scout.scoutInfo.general && !scout.scoutInfo.general?.opponent)
			scout.scoutInfo.general.opponent = {}
	})
		...
		<div class="mt-2 pl-2">
			{#if !!scout.scoutInfo.general?.opponent}
				...
				{#if loadingUpdateScoutInfo}
					<CircularLoader --circular-loader-height="20px" --circular-loader-width="20px"
					></CircularLoader>
				{:else}
			...
			<div class="flex flex-col gap-2 mt-8">
				{#each scout.players.filter((p) => p.isOpponent) as opponentPlayer}
					<div
						class="flex items-center gap-4 hover:bg-[rgb(var(--global-color-background-300))] p-1"
					>
						<div class="pl-2 flex-shrink">
							<button
								onclick={() => deletePlayer(opponentPlayer)}
								disabled={loadingDelete !== undefined}
							>
