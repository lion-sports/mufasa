			...
			if (phase === 'defenseBreak' || phase === 'defenseSideOut') {
				if (!!friendsPosition) {
					for (const [position, player] of Object.entries(friendsPosition)) {
						let exisitingPlayerIndex = playersCoordinates.findIndex(
							(pp) => pp.player.id == player.player.id
						)
						...
						if (exisitingPlayerIndex !== -1)
							playersCoordinates[exisitingPlayerIndex].coordinates = coordinates
						else {
							playersCoordinates = [
								...playersCoordinates,
								{
...
<SoftShadows focus={10} size={10} samples={40} />
...
{#if false}
	...
	<T.Mesh position={[0, 0, netHeight - 1]}>
		...
		<T.MeshStandardMaterial color={lineColor} transparent={true} opacity={0.5} />
	</T.Mesh>
{/if}
...
{#each playersCoordinates as playerCoordinate}
	<StudioField3DPlayerMarker
		position={[
			playerCoordinate.coordinates[0],
			playerCoordinate.coordinates[1],
			playerMarkerDimension + 1.5
		]}
		dimension={!!selectedPlayer && selectedPlayer.id == playerCoordinate.player.id
			? playerMarkerDimension * 1.5
			: playerMarkerDimension}
