<script lang="ts">
	...
	let allPlayers = $derived(
		lodash.uniqBy(
			orderedReceiveOverTimeByPlayer.map((e) => e.player),
			(e) => e.id
		)
	)
		...
		if (!!orderedReceiveOverTimeByPlayer) {
			...
			for (let i = 0; i < orderedReceiveOverTimeByPlayer.length; i += 1) {
				...
				for (let k = 0; k < chartData.datasets.length; k += 1) {
					if (k === datasetIndex)
						chartData.datasets[k].data = [...chartData.datasets[k].data, row.resultValue]
					else chartData.datasets[k].data = [...chartData.datasets[k].data, null]
				}
			}
		}
	})
...
<LineChart data={chartData} enableZoom={true} bind:resetZoom showYTicks showXTicks xTickStepSize={1}
></LineChart>
