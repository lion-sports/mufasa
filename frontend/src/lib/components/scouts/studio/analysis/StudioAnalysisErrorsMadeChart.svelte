<script lang="ts">
	...
	run(() => {
		if (!!dataGroupedByPlayer && groupBy == 'none') {
			...
			for (const [playerId, row] of Object.entries(dataGroupedByPlayer)) {
				let spikePoints =
					errorsMade.find((pm) => pm.category == 'spike' && pm.player.id == Number(playerId))
						?.errorsMade || 0
				let blockPoints =
					errorsMade.find((pm) => pm.category == 'block' && pm.player.id == Number(playerId))
						?.errorsMade || 0
				let servePoints =
					errorsMade.find((pm) => pm.category == 'serve' && pm.player.id == Number(playerId))
						?.errorsMade || 0
