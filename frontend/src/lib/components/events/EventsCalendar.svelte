<script lang="ts">
  ...
  import { theme, mediaQuery } from '@likable-hair/svelte';
      ...
      if(selectedView === 'timeGridWeek') {
        ...
        if(startYear === endYear) {
          if(endMonth == startMonth) {
            return { html: `<div style="font-size: 1.3rem">${startDate} - ${endDate} ${startMonth} ${startYear}</div>` }
          } else {
            return { html: `<div style="font-size: 1.3rem">${startDate} ${startMonth} - ${endDate} ${endMonth} ${startYear}</div>` }
          }
        } else {
          return { html: `<div style="font-size: 1.3rem">${startDate} ${startMonth} ${startYear} - ${endDate} ${endMonth} ${endYear}</div>` }
        }
      } else {
