...
import {
	OPPOSITE_POSITIONS,
	ORDERED_POSITIONS,
	type BlockScoutEventResult,
	type ReceiveScoutEventResult,
	type RotationType,
	type ServeScoutEventResult,
	type SpikeScoutEventResult,
	type VolleyballPhase,
	type VolleyballPlayersPosition,
	type VolleyballScoutEventJson,
	type VolleyballScoutEventParameters,
	type VolleyballScoutEventPosition
...
const handleStashReload = (data: { scout: Scout }) => {
...
const handleAnalysisReload = (data: { analysis: ScoutAnalysis }) => {
	studio.update((v) => {
		if (!!v) {
...
studio.subscribe((value) => {
	if (!!value) {
		...
		socketService.off(
			`teams:${value.scout.event.teamId}:scout:lastEventReload`,
			handleLastEventReload
		)
		socketService.on(
			`teams:${value.scout.event.teamId}:scout:lastEventReload`,
			handleLastEventReload
		...
		socketService.off(
			`teams:${value.scout.event.teamId}:scout:analysisReload`,
			handleAnalysisReload
		)
...
export function selectPlayer(params: { player: ScoutEventPlayer }) {
	studio.update((v) => {
		if (!!v) {
...
export async function add(params: { event: VolleyballScoutEventParameters }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')
...
export async function manualPhase(params: { phase: VolleyballPhase }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')
...
export async function teamRotation(params: { opponent: boolean; rotationType?: RotationType }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')

	let fromPositions: VolleyballPlayersPosition | undefined =
		currentStudio.scout.stash?.playersServePositions
...
export async function pointScored(params: { opponent: boolean }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')
...
export async function block(params: { player: ScoutEventPlayer; result: BlockScoutEventResult }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')
...
export async function serve(params: { player: ScoutEventPlayer; result: ServeScoutEventResult }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')
...
export async function spike(params: { player: ScoutEventPlayer; result: SpikeScoutEventResult }) {
	let currentStudio = get(studio)
	if (!currentStudio) throw new Error('cannot call without a studio')
...
export function getPlayerPositions(params: {
	...
	if (!!params.playerPositions) {
		...
		for (const [pos, value] of Object.entries(currentPosition)) {
			if (Number(value.player.id) == Number(params.player.id))
				return Number(pos) as VolleyballScoutEventPosition
		}
	}
...
export function suggestPlayer(params: {
	...
	let next2SetterPosition =
		ORDERED_POSITIONS[(orderOfSetterPosition + 2) % ORDERED_POSITIONS.length]

	if (params.position == setterPosition) {
		result = params.players.filter((p) => p.role == 'setter')
	} else if (
		params.position == nextSetterPosition ||
		params.position == OPPOSITE_POSITIONS[nextSetterPosition]
	) {
		result = params.players.filter((p) => p.role == 'outsideHitter')
	} else if (
		params.position == next2SetterPosition ||
		params.position == OPPOSITE_POSITIONS[next2SetterPosition]
	) {
