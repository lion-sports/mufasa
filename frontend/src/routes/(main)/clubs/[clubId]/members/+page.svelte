<script lang="ts">
  ...
  import type { PageData } from './$types';
	import { Icon, MediaQuery, SimpleTextField } from '@likable-hair/svelte'
          ...
          {:else}
            <div class="flex flex-col">
              <div class="flex items-start">
                {#if mAndDown}
                  <div class="mr-2 mt-2">
                    <button onclick={() => { selectedMember = undefined }}>
                      <Icon name="mdi-arrow-left" --icon-size="24px"></Icon>
                    </button>
                  </div>
                {/if}
                <div class="flex-grow">
