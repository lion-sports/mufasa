<script lang="ts">
	...
	function isScoringSystemValid(
		...
		return (
			...
			((ss.config.points.mode == 'totalPoints' && !!ss.config.points.totalPoints) ||
				(ss.config.points.mode == 'winPoints' && !!ss.config.points.winPoints))
		)
	}
	...
	let scoringSystem: NonNullable<ComponentProps<typeof ScoringSystemForm>['scoringSystem']> =
			$state({}),
...
<PageTitle title={scoringSystem.name || 'Nuovo sistema di punteggio'} prependVisible></PageTitle>

<ScoringSystemForm bind:scoringSystem bind:formValid></ScoringSystemForm>
